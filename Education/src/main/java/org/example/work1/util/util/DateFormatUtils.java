...
public class DateFormatUtils {
	...
	public static int countWorkingDays(Date startDate, Date endDate) {
		int count = 0;
		LocalDate starLocalDate = LocalDateTime.ofInstant(startDate.toInstant(), ZoneId.systemDefault()).toLocalDate();
		LocalDate endLocalDate = LocalDateTime.ofInstant(endDate.toInstant(), ZoneId.systemDefault()).toLocalDate();
		while (starLocalDate.isBefore(endLocalDate) || starLocalDate.isEqual(endLocalDate)) {
			DayOfWeek dayOfWeek = starLocalDate.getDayOfWeek();
